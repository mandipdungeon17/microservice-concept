...
**Q103: "Why must GiftSaga compensation use explicit `completedSteps` instead of reading current status after setting `COMPENSATING`?" (2026-08-14, Topic 2)**

A: Because the act of entering compensation overwrites the previous forward status. If logic sets `status=COMPENSATING` first and then asks "which step completed?", the answer is lost, so undo steps may be skipped. Explicit `completedSteps` preserves the exact partial commit boundary (0/1/2/3) from the failure context. That makes compensation deterministic and safe under retries and timeout recovery.

**Q104: "Is Stock Gifting Saga part of normal buy/sell flow?" (2026-08-14, Topic 2)**

A: No. It is a separate, user-triggered transfer flow (`giver -> receiver`) and should execute only on the dedicated gift endpoint. Buy/sell flows represent market trade/payment behavior; gifting represents peer-to-peer holding transfer. Keeping them separate avoids accidental coupling and prevents gifting logic from running on every trade.

**Q105: "Why do we still need idempotency key when sagaId already exists?" (2026-08-14, Topic 2)**

A: `sagaId` is generated by server after request acceptance, so it cannot dedupe retries that happen before client receives response. `idempotencyKey` is client-supplied and stable across retries, so server can short-circuit duplicate requests and return the existing saga state. This prevents double debit/credit under network retries.

**Q106: "Why do we run timeout detection for GiftSaga if compensation already exists in catch blocks?" (2026-08-14, Topic 2)**

A: Catch-block compensation handles in-process failures. Timeout detection handles crash/restart or node-loss scenarios where process dies mid-saga and no catch block executes. Persisted saga status + scheduler scan is the recovery mechanism for failures outside the current JVM lifecycle.
