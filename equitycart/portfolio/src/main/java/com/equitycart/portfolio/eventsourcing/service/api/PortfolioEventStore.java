...
/**
 * Append-only event store interface for portfolio state changes.
 * Implementations persist events to
 * MongoDB as immutable documents, providing a complete audit trail and enabling
 * state
 * reconstruction via projection replay.
 *
 * <p>
 * Designed as best-effort: failures to append do NOT propagate to the caller —
 * the primary
 * PostgreSQL state update proceeds regardless. This prevents MongoDB outages
 * from breaking core
...
public interface PortfolioEventStore {
  ...
  /**
   * Appends a new event to the store. Assigns a monotonically increasing sequence
   * number per user
   ...
   * @param tickerSymbol  affected stock (null for non-holding informational
   *                      events)
   * @param quantity      shares involved in the operation
   * @param pricePerShare execution price (null or ZERO for reward events)
   * @param totalValue    monetary value of the operation (quantity × price, or
   *                      reward dollar value)
